...
NBA.mc <- NBA.stdz %>% 
  bind_cols(as_tibble(NBA.pca$scores[, 1:2])) %>% 
  na.omit() %>% 
  select(c("Player", "Pos", "Comp.1", "Comp.2", cols.to.explore)) %>% 
  filter(Pos != 'PF-C')

# fit one model per position
mc.clusts <- lapply(X = split(NBA.mc[, -(1:4)], NBA.mc$Pos), FUN = function(x) Mclust(data = x, G = 4)$classification)

# bind back the cluster groups with the data
NBA.mc <- lapply(names(mc.clusts), function(df){
  bind_cols(split(NBA.mc, NBA.mc$Pos)[[df]],
            enframe(mc.clusts[[df]]))
}) %>% bind_rows() %>% select(-name)
