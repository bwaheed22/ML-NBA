...
author: "Andrew Pagtakhan, Joe Marlo, Bilal Waheed, George Perrett"
...
players.to.show <- c(players_cl_one[1], players_cl_two[1],
                 players_cl_three[1], players_cl_four[1])

players.to.show <- c("Harrison Barnes", "Trey Lyles", "James Johnson")
```
...
salary.diff.df <- nba %>%
  ...
  mutate(Salary_diff = (Clust_Salary - Salary) / 1000000,
         ...
         Label = if_else(Show, paste0(Player, ", Cluster ", km_labs_four), ""),
         Color = if_else(Show, "grey80", NULL)) %>% 
  arrange(Salary_diff)

# plot it
# salary.diff.df %>% 
#   ggplot(aes(x = reorder(Player, Salary_diff), y = Salary_diff,
#              color = Salary_diff, labels = Label)) +
#   geom_point() +
#   geom_hline(yintercept = 0,
#              linetype = "dashed",
#              color = "grey40") +
#   scale_x_discrete(breaks = salary.diff.df$Label) +
#   # scale_x_discrete(labels = salary.diff.df$Label) +
...
salary.diff.df %>% 
  ggplot(aes(y = reorder(Player, Salary_diff), x = Salary_diff,
             color = Salary_diff, labels = Label)) +
  geom_segment(aes(xend = -11,
                   yend = seq_along(Player)),
               color = salary.diff.df$Color,
               size = 0.3) +
  geom_text(aes(x = -15,
                label = Label),
                color = "grey50",
  ...
  # scale_y_discrete(breaks = salary.diff.df$Label) +
